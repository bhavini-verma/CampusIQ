...
export const DELETE = withAuth(
  async (
    ...
    if (isNaN(targetId)) {
      return errorResponse("Invalid ID. Must be an integer.", 400);
    }

    try {
      // 1. Try to find the record assuming the ID refers to the College ID
      ...
      // 2. If not found, assume targetId refers to the SavedCollege record ID directly
      if (!savedRecord) {
      ...
      if (!savedRecord) {
        return errorResponse(
          `No saved college found for College/Saved ID ${targetId}`,
          404
        );
      }
