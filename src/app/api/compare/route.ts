...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (colleges.length >= 2) {
      ...
      const placementDiffVal = (
        placementWinner.placementAvgSalary - placementRunnerUp.placementAvgSalary
      ).toFixed(1);

      comparisonResults.diff.placement = {
        winner: placementWinner.name,
        by:
          parseFloat(placementDiffVal) > 0
            ? `₹${placementDiffVal}L higher avg`
      ...
      // 4. CUTOFF RANK (Lower is better - more selective/competitive)
      ...
      // 5. Intelligent Verdict Engine (weighted values to calculate the overall winner)
      const scores = new Map<number, number>();
      colleges.forEach((c) => scores.set(c.id, 0));

      // Add scores based on category wins
      scores.set(feesWinner.id, (scores.get(feesWinner.id) || 0) + 1.0);
      scores.set(placementWinner.id, (scores.get(placementWinner.id) || 0) + 1.5); // Weighted higher
      scores.set(ratingWinner.id, (scores.get(ratingWinner.id) || 0) + 1.0);
      scores.set(cutoffWinner.id, (scores.get(cutoffWinner.id) || 0) + 1.2); // Weighted higher
      ...
      // Construct dynamic verdict reasoning
      let verdictReasoning = "";
      if (overallWinner.id === placementWinner.id && overallWinner.id === ratingWinner.id) {
        verdictReasoning = `${overallWinner.name} offers the best overall value, leading both in premium placements (avg ₹${overallWinner.placementAvgSalary}L) and superb student ratings (${overallWinner.rating} stars).`;
    ...
    // Optional: Log comparison and save under user account
